...
static void Com_GenerateMediaTXT_f(void)
{
	...
	FS_Printf(f, "// generated by XreaL\n\n"
			  ...
			  "Used CC licenses are listed below:\n\n"
	...
	Q_strncpyz(license, "LICENSE", sizeof(license));
	Q_strncpyz(source, "SOURCE", sizeof(source));
	...
	while(qtrue)
	{
		...
		if(!Q_stricmp(token, "properties") || !Q_stricmp(token, "eigenschaften"))
		{
			   ...
			   Q_stricmp(mediaName + len - 3, ".sh") != 0 && Q_stricmp(mediaName + len - 4, ".mtr") != 0)
			{
				...
				if(firstEntry)
				{
					...
					// do a separator line
					len = 0;
					while(len < (maxMediaLen + maxCopyrightLen + maxLicenseLen + 6))
					{
						FS_Printf(f, "=");
						len++;
					}
			...
			mediaName[0] = '\0';
			Q_strncpyz(copyright, "<add author>", sizeof(copyright));
			Q_strncpyz(license, "All Rights Reserved", sizeof(license));
			source[0] = '\0';
		...
		else
		{
			...
			{
				Q_strncpyz(copyright, propertyValue, sizeof(copyright));
			}
			else if(!Q_stricmp(propertyName, "svn:license"))
			{
				Q_strncpyz(license, propertyValue, sizeof(license));
			}
			else if(!Q_stricmp(propertyName, "svn:source"))
			{
				Q_strncpyz(source, propertyValue, sizeof(source));
			}
		}
	}
...
static void Field_CompleteKeyname(void)
{
	...
	Q_strncpyz(&completionField->buffer[strlen(completionField->buffer) -
										strlen(completionString)], shortestMatch, sizeof(completionField->buffer));
...
===============
*/
static void Field_CompleteFilename(const char *dir, const char *ext, qboolean stripExt)
{
	matchCount = 0;
	shortestMatch[0] = 0;
...
===============
*/
static void Field_CompleteCommand(char *cmd, qboolean doCommands, qboolean doCvars)
{
	...
	if(completionArgument > 1)
	{
		...
		else
		{
			...
			if((!Q_stricmp(baseCmd, "map") ||
				...
				!Q_stricmp(baseCmd, "spmap") || !Q_stricmp(baseCmd, "spdevmap")) && completionArgument == 2)
			{
			...
			else if(!Q_stricmp(baseCmd, "condump") && completionArgument == 2)
			{
				Field_CompleteFilename("", "txt", qfalse);
					 ...
					 !Q_stricmp(baseCmd, "setu") || !Q_stricmp(baseCmd, "sets")) && completionArgument == 2)
			{
				// Skip "<cmd> "
