...
TextureGroups TextureGroups_constructTreeView()
{
  ...
  else
  {
    ...
    if (g_pGameDescription->mGameType != "doom3" && g_pGameDescription->mGameType != "xreal" && !g_TextureBrowser_shaderlistOnly)
    {
...
GtkMenuItem* TextureBrowser_constructViewMenu(GtkMenu* menu)
{
  ...
  if(g_pGameDescription->mGameType == "doom3" || g_pGameDescription->mGameType == "xreal" || !string_empty(g_pGameDescription->getKeyValue("show_wads")))
  {
