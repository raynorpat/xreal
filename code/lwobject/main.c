...
static void print_vmaps1(FILE * fp, lwLayer * layer)
{
	...
	for(i = 0; i < layer->nvmaps; i++)
	{
		...
		fprintf(fp, "%c%c%c%c \"%s\"  dim %d  nverts %d  vmad (%s)\n",
				tag[3], tag[2], tag[1], tag[0], vmap->name, vmap->dim, vmap->nverts, vmap->perpoly ? "yes" : "no");
		...
		for(j = 0; j < vmap->nverts; j++)
		{
			...
			if(vmap->perpoly)
			{
				...
				for(n = 0; n < pol->nverts; n++)
					if(pol->v[n].index == vmap->vindex[j])
						break;
				fprintf(fp, " vert %d ", n);
			}
...
static void print_vmaps2(FILE * fp, lwLayer * layer)
{
	...
	for(i = 0; i < layer->point.count; i++)
	{
		...
		for(j = 0; j < pt->nvmaps; j++)
		{
			...
			fprintf(fp, "  %c%c%c%c \"%s\" vmad (%s)", tag[3], tag[2], tag[1], tag[0], vmap->name, vmap->perpoly ? "yes" : "no");
...
static void print_vmaps3(FILE * fp, lwLayer * layer)
{
	...
	for(i = 0; i < layer->polygon.count; i++)
	{
		...
		tag = (char *)&pol->type;
		fprintf(fp, "%d %c%c%c%c  nverts %d\n", i, tag[3], tag[2], tag[1], tag[0], pol->nverts);
		...
		for(k = 0; k < pol->nverts; k++)
		{
			...
			fprintf(fp, "%d (%g, %g, %g)  nvmads %d  nvmaps %d\n", n,
					pt->pos[0], pt->pos[1], pt->pos[2], pol->v[k].nvmaps, pt->nvmaps - pol->v[k].nvmaps);
			...
			for(j = 0; j < pol->v[k].nvmaps; j++)
			{
				...
				tag = (char *)&vmap->type;
				fprintf(fp, "  %c%c%c%c vmad \"%s\"", tag[3], tag[2], tag[1], tag[0], vmap->name);
			...
			for(j = 0; j < pt->nvmaps; j++)
			{
				...
				tag = (char *)&vmap->type;
				fprintf(fp, "  %c%c%c%c vmap \"%s\"", tag[3], tag[2], tag[1], tag[0], vmap->name);
